...
void mergesort_helper(char *str, char *scratch, int low, int high) {
    ...
    middle = (high - low) / 2 + low;  // Don't add low to middle

    if(low != high){ // Not the right one
        mergesort_helper(str, scratch, low, middle);  // Condition to make sure low and middle aren't equal
...
void mergesort(char *str, char *scratch) {
    mergesort_helper(str, scratch, 0, strlen(str)-1);  // Strlen(is one to high)
}
...
int main(int argc, char **argv) {
    if (argc != 2) {
        ...
        char scratch[strlen(argv[1])]; // Needs to be argv[1], strlen(1) = 11
        mergesort(argv[1], scratch);  // Needs to be argv[1]
        printf("%s\n", argv[1]);  // Needs to be argv[1]
        return 0;
    }
}
